...
import { allPolicies, policyGroups, policyMetrics, siteCutoff, weeklyChanges } from "./policy-data";
...
export default function Home() {
  return <main>
    <section className="hero"><p className="eyebrow">BEIJING · AI POLICY INTELLIGENCE</p><div className="hero-grid"><div><h1>北京 AI<br />政策情报</h1><p className="intro">覆盖国家、北京各层级及重点外省市的 AI、大模型、算力与数据政策；每周核验、分类沉淀、动态维护。</p></div><aside className="as-of"><span>本期截止</span><strong>{siteCutoff.date}</strong><p>中国标准时间 · 第 {siteCutoff.isoWeek} 周</p></aside></div></section>
    <SiteNav />
    <section className="overview"><div className="section-heading"><p className="eyebrow">POLICY LANDSCAPE</p><h2>政策全景</h2><p>首页呈现整体态势；完整政策请进入分类页查看。</p></div><div className="overview-grid"><article><strong>{policyCount}</strong><span>项政策基线</span></article><article><strong>{policyGroups.length}</strong><span>个政策层级</span></article><article><strong>{themeCount}</strong><span>个核心主题</span></article><article><strong>{weeklyChanges.length}</strong><span>项本周变化</span></article></div><div className="level-links">{policyGroups.map((group) => <a key={group.id} href={`/${group.id}`}><span>{group.title}</span><strong>{group.policies.length} 项</strong><p>{group.note}</p><i>查看政策库 ↗</i></a>)}</div></section>
