...
import { PolicyAnalysisSections } from "../components/PolicyAnalysisSections";
import { getPolicyById, policyGroups } from "../policy-data";

export const metadata: Metadata = {
  title: "政策分析｜北京 AI 政策情报",
  description: "面向管理层的北京 AI 政策依据链、趋势预警、重点机会、风险与行动建议。",
};

export default function LeadershipPage() {
  return <main>
    <section className="audience-hero leadership-hero">
      <p className="eyebrow">POLICY ANALYSIS · 2026-08-03</p>
      <div>
        <h1>政策分析</h1>
        <p>用一页快速掌握政策方向、依据链、趋势预警、风险边界和需要推动的管理动作。</p>
      </div>
    ...
    <section className="audience-section leadership-summary">
      ...
      <div className="signal-grid">{leadershipSignals.map((item, index) => <article key={item.title}><span className="signal-number">{String(index + 1).padStart(2, "0")}</span><h3>{item.title}</h3><p>{item.signal}</p><strong>{item.managementMeaning}</strong><div className="policy-link-row">{item.policyIds.map((policyId) => { const policy = getPolicyById(policyId); return <a key={policyId} href={policyHref(policyId)}>{policy?.title ?? policyId} ↗</a>; })}</div></article>)}</div>
    </section>

    <PolicyAnalysisSections />
    ...
    <footer>北京 AI 政策情报 · 政策分析视图 · 基于公开政策的业务研判</footer>
  </main>;
}
